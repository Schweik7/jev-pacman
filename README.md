...
| 规则基线（回合制，通关） | 模拟 Jev（实时，约 300ms 延迟） | 真 Jev（实时 600ms 节拍，5 倍速播放） |
|---|---|---|
...
- `--gif out.gif`：把第一局用 Pillow 画成 GIF。`--gif-every` 控制隔几个 tick 取一帧，
  `--gif-ms` 覆盖每帧毫秒数（默认按真实节奏；上面那张 600ms 节拍的图就是 `--gif-ms 120`，即 5 倍速）。

`--watch` 的画面只会出现在跑命令的那个终端里，输出重定向到文件就看不到了；想留档就用 `--gif`。
...
|---|---|---|---|---|
| 回合制 | — | 2260 分，194/244 豆，剩 2 命（到 300 tick 上限） | 705 / 1220 ms | — |
| 实时 | 600ms | 1520 分，128/244 豆，235 tick 丢光命 | 710 / 1211 ms | 1.26 tick |
| 实时 | 600ms | 920 分，92/244 豆，剩 1 命（到 300 tick 上限） | 860 / 1332 ms | 1.54 tick |
| 实时 | 200ms | 530 分，146 tick 丢光命 | 855 / 4747 ms | 6.64 tick |

（两局 600ms 用的是同一个 seed：实时模式下延迟抖动会让轨迹分叉，单局结果方差不小。）
